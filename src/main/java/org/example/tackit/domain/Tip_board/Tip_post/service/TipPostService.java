...
public class TipPostService {
    ...
    public PageResponseDTO<TipPostRespDto> getActivePostsByOrganization(Long orgId, Pageable pageable) {
    ...
    // [ 게시글 작성 ] : 선배만 가능
    @Transactional
    public TipPostRespDto createPost(TipPostReqDto dto, String email, Long orgId, MultipartFile image) throws IOException {
        // 1. 유저 조회
    ...
    // [ 게시글 수정 ] : 작성자만
    @Transactional
    public TipPostRespDto update(Long id, TipPostUpdateDto dto, String email, Long orgId, MultipartFile image) throws IOException {
        MemberOrg member = memberOrgRepository.findByMemberEmailAndId(email, orgId)
                .orElseThrow(() -> new MemberNotFoundException(ErrorCode.MEMBER_NOT_FOUND));
    ...
    public List<TipPopularPostRespDto> getPopularPosts(Long orgId) {
        List<TipPost> popularPosts = tipPostRepository.findTop3ByWriterIdOrderByViewCountDescScrapCountDesc(orgId);
