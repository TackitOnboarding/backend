...
public class MyPageQnAService {
    ...
    public PageResponseDTO<QnAMyPostResponseDto> getMyPosts(String email, Long orgId, Pageable pageable) {
        MemberOrg member = memberOrgRepository.findByMemberEmailAndId(email, orgId)
                .orElseThrow(() -> new IllegalArgumentException("존재하지 않는 사용자입니다."));

        Page<QnAPost> postPage = qnAPostRepository.findByWriterAndAccountStatus(member, AccountStatus.ACTIVE, pageable);
    ...
    // 내가 쓴 댓글 조회
    @Transactional(readOnly = true)
    public PageResponseDTO<QnAMyCommentResponseDto> getMyComments(String email, Long orgId, Pageable pageable) {
        MemberOrg member = memberOrgRepository.findByMemberEmailAndId(email, orgId)
                .orElseThrow(() -> new IllegalArgumentException("존재하지 않는 사용자입니다."));
